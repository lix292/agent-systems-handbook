...
这个入门项目把客户支持案例研究扩展成一个接入邮箱的本地工作流：
同步带标签的 Gmail 线程，先用确定性规则分类客户问题，再把它们写入
SQLite，并通过本地 dashboard 审阅或发送回复。
...
但加入了团队下一步通常需要的运行面：Gmail 同步、本地 SQLite 状态、
客户审核队列，以及一个确定性的 dashboard。
...
对于 connector 辅助的低 token 路径，优先把批次写入文件，而不是把大段 JSON
直接打印到 stdout：
...
- 硬逻辑负责 Gmail 抓取、HTML 清洗、签名裁剪、引用历史移除、客户匹配、
  SQLite 持久化、分析统计和发送队列执行。
...
- `src/lib/sync.ts`：确定性的发送队列、来信批次、草稿批次和草稿渲染协调逻辑
- `app/page.tsx`：包含 issues、analysis、customer review 和 customer CRUD 标签页的 Next.js dashboard

## 约束

- Gmail 集成需要真实的 OAuth 环境变量。
- v1 忽略附件和内联图片。
- 该 dashboard 面向单操作员和本地机器。
- `npm run sync` 使用确定性的理解和起草兜底逻辑，因此即使没有实时模型调用也能运行。
